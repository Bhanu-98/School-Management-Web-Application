		...
		<?php include"navbar.php";?><br>
			<div id="section">
				...
				<div class="content">
					...
					<?php
						if(isset($_POST["submit"]))
						{
							...
							{
								$sq="insert into student(RNO,NAME,FNAME,DOB,GEN,PHO,MAIL,ADDR,SCLASS,SSEC,SIMG,TID,SPASS,bus) values('{$_POST["rno"]}','{$_POST["name"]}','{$_POST["fname"]}','{$edate}','{$_POST["gen"]}','{$_POST["pho"]}','{$_POST["email"]}','{$_POST["addr"]}','{$_POST["cla"]}','{$_POST["sec"]}','{$target_file}','{$_SESSION["TID"]}','vsr','{$_POST["bus"]}')";
								
								if($db->query($sq))
								{
									echo "<div class='success'>Student details inserted</div>";
								}
								else
								{
				...
				<label> Parent's Mail Id</label><br>
					...
					<label>Does he require bus facility</label>
					<select name="bus" required class="input3">
							<option value="">Select</option>
			...
			<button type="submit" style="float:centre;" class="btn" name="submit">Add Student Details</button>
				</div>
